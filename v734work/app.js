...
async function runSystemCheck(){
  ...
  const coreIssues=runCoreSelfCheck();checks.push(['画面・ボタン整合性',coreIssues.length===0,coreIssues.length?coreIssues.slice(0,3).join(' / '):'主要画面・リンクOK']);checks.push(['ビルド',VERTX_BUILD==='7.35.0',`v${VERTX_BUILD}`]);
...
function materialCard(m){const q=state.cart[m.id]||0,f=state.favorites.has(m.id),viewer=getEffectiveRole()==='viewer';const spec=(m.aliases||'').trim();return `<article class="material ${q?'selected':''} ${viewer?'viewer-material':''}"><div class="material-actions"><button class="fav-btn ${f?'active':''}" data-fav="${m.id}" aria-label="お気に入り" title="お気に入り">☆</button>${viewer?'':`<button class="material-more" data-material-edit="${m.id}" aria-label="資材名・単重・カテゴリーを変更"><span>•••</span><small>編集</small></button>`}</div><div class="material-info"><b>${escapeHtml(m.name)}</b>${spec?`<small class="material-spec">${escapeHtml(spec)}</small>`:''}<small>${escapeHtml(m.category)} · ${Number(m.weight).toFixed(2)}kg/${escapeHtml(m.unit)}</small></div>${viewer?'<div class="viewer-material-badge">閲覧のみ</div>':`<div class="qty-control"><button data-action="minus" data-id="${m.id}">−</button><input data-qty="${m.id}" inputmode="numeric" value="${q}" aria-label="${escapeHtml(m.name)}数量"><button data-action="plus" data-id="${m.id}">＋</button></div>`}</article>`}
...
function openMaterialQuickEdit(id){if(getEffectiveRole()==='viewer')return toast('閲覧権限では資材設定を変更できません');const m=MATERIALS.find(x=>String(x.id)===String(id));const modal=$('#materialQuickEdit');if(!m||!modal)return;modal.dataset.materialId=m.id;$('#quickMaterialOriginal').textContent=`標準名 / ${m.aliases?.split('/')[0]?.trim()||m.name}`;$('#quickMaterialName').value=m.name;$('#quickMaterialWeight').value=Number(m.weight||0);$('#quickMaterialCategory').value=m.category||'その他';$('#quickMaterialHidden').checked=!!m.hidden;modal.classList.remove('hidden');document.body.classList.add('modal-open')}
...
function changeQty(id,d){if(getEffectiveRole()==='viewer')return;state.cart[id]=Math.max(0,(state.cart[id]||0)+d);if(d>0)rememberMaterial(id);persistDraftState();renderMaterials();if($('#favorites').classList.contains('active'))renderFavorites()}
function setQty(id,v){if(getEffectiveRole()==='viewer')return;state.cart[id]=Math.max(0,Math.floor(Number.isFinite(v)?v:0));if(state.cart[id]>0)rememberMaterial(id);persistDraftState();renderMaterials();if($('#favorites').classList.contains('active'))renderFavorites()}
...
function renderSites(){const viewer=getEffectiveRole()==='viewer',sites=getSites();$('#siteList').innerHTML=sites.length?sites.map((s,i)=>`<article class="card site-card"><button class="site-select" data-site="${escapeHtml(s)}"><span>📍</span><b>${escapeHtml(s)}</b><small>${viewer?'この現場の資材を確認':'この現場で注文を作る'}</small></button>${viewer?'':`<button class="site-delete" data-site-index="${i}">×</button>`}</article>`).join(''):'<div class="card empty">登録現場はまだありません</div>';$$('[data-site]').forEach(b=>b.onclick=()=>{state.selectedSite=b.dataset.site;persistDraftState();if($('#siteName'))$('#siteName').value=state.selectedSite;go('order')});if(!viewer)$$('[data-site-index]').forEach(b=>b.onclick=()=>{const a=getSites();a.splice(Number(b.dataset.siteIndex),1);saveSites(a);renderSites()})}
function addSite(){if(getEffectiveRole()==='viewer')return toast('閲覧権限では現場を追加できません');const name=$('#newSiteName').value.trim();if(!name)return toast('現場名を入力してください');const sites=getSites();if(!sites.includes(name))sites.unshift(name);saveSites(sites);audit('現場追加',name);$('#newSiteName').value='';renderSites();toast('現場を追加しました')}
...
function canOpenScreen(screenId){const actual=getActualRole();if(screenId==='devTest')return actual==='owner';const r=getEffectiveRole();if(r==='owner')return true;if(r==='admin')return !['plans','devTest'].includes(screenId);if(r==='member')return !['materialsMaster','members','plans','analytics','companySettings','siteCosts','suppliers','auditLog','devTest'].includes(screenId);if(r==='viewer')return ['home','more','history','favorites','drawings','siteStock','siteDashboard','siteQr','companySettings','manual','returnLoad','dailyReport','sites','order','assist'].includes(screenId);return false}
function applyViewerReadOnly(screenId){
  const viewer=getEffectiveRole()==='viewer';
  document.body.classList.toggle('viewer-mode',viewer);
  if(!viewer)return;
  if(screenId==='sites'){
    const form=document.querySelector('#sites .compact-form'); if(form)form.style.display='none';
    let n=document.getElementById('viewerSitesNotice');
    if(!n){n=document.createElement('div');n.id='viewerSitesNotice';n.className='card viewer-readonly-notice';n.innerHTML='<b>閲覧モード</b><span>現場の確認はできます。追加・削除・注文作成はできません。</span>';document.querySelector('#sites .screen-head')?.insertAdjacentElement('afterend',n)}
  }
  if(screenId==='order'){
    let n=document.getElementById('viewerOrderNotice');
    if(!n){n=document.createElement('div');n.id='viewerOrderNotice';n.className='card viewer-readonly-notice';n.innerHTML='<b>閲覧モード</b><span>資材名・規格・重量を確認できます。数量変更や注文保存はできません。</span>';document.querySelector('#order .screen-head')?.insertAdjacentElement('afterend',n)}
    document.querySelectorAll('#order [data-action],#order [data-qty],#order [data-material-edit],#order [data-quick-add],#order [data-site-smart]').forEach(el=>{el.disabled=true;el.setAttribute('aria-disabled','true')});
    const confirm=document.getElementById('toConfirmBtn'); if(confirm){confirm.disabled=true;confirm.textContent='閲覧専用'}
    const voice=document.querySelector('#order [data-go="voiceOrder"]'); if(voice)voice.style.display='none';
  }
  if(screenId==='assist'){
    let n=document.getElementById('viewerAiNotice');
    if(!n){n=document.createElement('div');n.id='viewerAiNotice';n.className='card viewer-readonly-notice';n.innerHTML='<b>閲覧モード</b><span>AI画面と保存済み結果は確認できます。新しい解析・注文反映はできません。</span>';document.querySelector('#assist .screen-head')?.insertAdjacentElement('afterend',n)}
    ['aiDrawingInput','selectAllDrawings','clearAllDrawings','aiMode','aiContext','runAiBtn','assistType','assistSpans','assistLevels','assistFrameWidth','assistSpanLength','assistPanels','runAssistBtn','applyAssistBtn','applyAiBtn'].forEach(id=>{const el=document.getElementById(id);if(el){el.disabled=true;el.setAttribute('aria-disabled','true')}});
    document.querySelectorAll('#assist .upload-btn').forEach(el=>el.classList.add('viewer-disabled'));
  }
}
...
const HOME_WEATHER_CACHE_KEY='vertx_core_home_weather_v736';
function homeDisplayName(){
  const s=getCompanySession()||{};
  const raw=String(s.user||getSavedIdentity()?.user||cloudUser?.user_metadata?.name||cloudUser?.email||'ユーザー').trim();
...
function renderHomeIdentityLive(){
  ...
  if($('#homeCompanyLive'))$('#homeCompanyLive').textContent=s.company||'VERTX CORE';
...
async function refreshHomeWeather(force=false){
  try{
    ...
    if(!force&&cached&&Date.now()-Number(cached.at||0)<30*60*1000){renderHomeWeatherData(cached);return}
  }catch{}
...
function openHomeLiveModal(kind){
  ...
  if(kind==='notice'){
    ...
    const s=getCompanySession()||{};eye.textContent='ACCOUNT';title.textContent='アカウント';body.innerHTML=`<div class="home-live-row"><div><b>${escapeHtml(homeDisplayName())}</b><small>${escapeHtml(s.company||'会社未設定')}</small></div><strong class="status-online">${navigator.onLine?'● ONLINE':'● OFFLINE'}</strong></div><div class="home-live-row"><div><b>${escapeHtml(roleLabel(getEffectiveRole()))}</b><small>現在の権限</small></div><strong>${escapeHtml((s.plan||'free').toUpperCase())}</strong></div><button class="gold" type="button" data-live-go="companySettings">会社・アカウント設定</button><button type="button" id="homeModalSignOut">ログアウト</button>`;body.querySelector('[data-live-go]')?.addEventListener('click',()=>{modal.classList.add('hidden');go('companySettings')});$('#homeModalSignOut')?.addEventListener('click',signOut);
  }
...
function renderDailyReport(){const role=getEffectiveRole();const viewer=role==='viewer';bindKyRiskInputs();const form=document.querySelector('#dailyReport .field-brief-card');if(form)form.classList.toggle('viewer-hidden',viewer);let notice=document.getElementById('dailyViewerNotice');if(viewer&&!notice){notice=document.createElement('div');notice.id='dailyViewerNotice';notice.className='card viewer-ky-notice';notice.innerHTML='<b>閲覧モード</b><span>職長日報とKYは確認できます。入力・変更はできません。</span>';const head=document.querySelector('#dailyReport .screen-head');head?.insertAdjacentElement('afterend',notice)}else if(notice){notice.classList.toggle('hidden',!viewer)}const current=$('#dailySite')?.value||state.selectedSite||lsGet('vertx_core_last_site')||'';if(!viewer){fillSiteSelect($('#dailySite'),current);if($('#dailyDate')&&!$('#dailyDate').value)$('#dailyDate').value=todayIso();if($('#dailyReportDateLabel'))$('#dailyReportDateLabel').textContent=$('#dailyDate').value||todayIso();renderDailyTruck()}const root=$('#dailyReportList');if(!root)return;const rows=getDailyReports();root.innerHTML=rows.length?rows.map(x=>`<article class="card daily-report-card"><div class="daily-report-head"><div><span>${escapeHtml(x.date)}</span><b>${escapeHtml(x.site)}</b></div><em>${Number(x.workerCount)||0}名</em></div><p><b>作業者</b> ${escapeHtml(x.workerNames||'未入力')}</p><p><b>作業</b> ${escapeHtml(x.work||'未入力')}</p><p><b>現状</b> ${escapeHtml(x.status||'未入力')}</p>${x.safetyGoal?`<div class="daily-safety-goal">重点安全目標：${escapeHtml(x.safetyGoal)}</div>`:''}<div class="daily-ky-summary">${Array.isArray(x.kyRisks)&&x.kyRisks.length?x.kyRisks.map((r,i)=>`<p><b>KY ${i+1}</b> ${escapeHtml(r.hazard||'')} <small>リスク${Number(r.risk)||0}</small><br><span>${escapeHtml(r.measure||'')}</span></p>`).join(''):`<p><b>KY・危険ポイント</b> ${escapeHtml(x.kyText||'未入力')}</p><p><b>対策・周知事項</b> ${escapeHtml(x.kyMeasure||'未入力')}</p>`}</div>${x.handoverImage?`<div class="handover-photo-note"><span>引継ぎ写真メモ</span><img src="${x.handoverImage}" alt="引継ぎ写真メモ"></div>`:''}${x.handoverTo?`<div class="handover-chip">引継ぎ → ${escapeHtml(x.handoverTo)} / ${escapeHtml(x.handoverStatus||'未確認')}</div>`:''}${!viewer&&x.handoverTo&&x.handoverStatus!=='確認済み'?`<button class="secondary-btn full" data-confirm-daily="${x.id}">引継ぎを確認済みにする</button>`:''}</article>`).join(''):'<div class="card empty">まだ日報はありません</div>';if(!viewer)root.querySelectorAll('[data-confirm-daily]').forEach(b=>b.onclick=()=>confirmDaily(Number(b.dataset.confirmDaily)))}
...
function buildClientSnapshot(site,notice=''){const daily=getLatestDailyForSite(site),photos=getSiteAlbum().filter(x=>x.site===site&&x.share==='yes').slice(0,5),today=todayIso(),deliveries=getOrders().filter(x=>x.site===site&&String(x.date||'')>=today).slice(0,6),returns=getReturnTruckRequests().filter(x=>x.site===site&&String(x.date||'')>=today&&x.status!=='返却完了').slice(0,6);const kyRisks=Array.isArray(daily?.kyRisks)?daily.kyRisks.slice(0,3).map(r=>({hazard:r.hazard||'',cause:r.cause||'',risk:Number(r.risk)||0,measure:r.measure||''})):[];return {site,status:daily?.status||'',notice,safetyGoal:daily?.safetyGoal||'',kyRisks,kyText:daily?.kyText||'',kyMeasure:daily?.kyMeasure||'',photos:photos.map(x=>({tag:x.tag,comment:x.comment,dataUrl:x.dataUrl,createdAt:x.createdAt})),deliveries:deliveries.map(x=>({date:x.date,time:x.time||'',truck:x.truck||truckFor(x.weight||0),status:x.status||'',weight:x.weight||0})),returns:returns.map(x=>({date:x.date,time:x.time||'',truck:x.truck||'',status:x.status||'',weight:x.weight||0})),orders:orders.map(o=>({date:o.date||'',status:o.status||'',items:(o.items||[]).slice(0,30).map(i=>({name:i.name,qty:i.qty,unit:i.unit}))})),updatedAt:new Date().toISOString()}}
...
function renderClientPreview(p){const root=$('#sharePreview');if(!root)return;if(!p){root.innerHTML='';return}const ky=(p.kyRisks||[]).length?(p.kyRisks||[]).map((r,i)=>`<div class="public-ky-row"><b>KY ${i+1}</b><span>${escapeHtml(r.hazard||'')}</span><small>${escapeHtml(r.measure||'')}</small></div>`).join(''):(p.kyText||p.kyMeasure)?`<div class="public-ky-row"><b>KY</b><span>${escapeHtml(p.kyText||'')}</span><small>${escapeHtml(p.kyMeasure||'')}</small></div>`:'';root.innerHTML=`<div class="public-preview-head"><span>共有プレビュー</span><h3>${escapeHtml(p.site)}</h3><p>${escapeHtml(p.status||'現場状況未入力')}</p></div>${p.safetyGoal?`<div class="public-preview-safety">重点安全目標：${escapeHtml(p.safetyGoal)}</div>`:''}${ky?`<div class="public-preview-ky">${ky}</div>`:''}<div class="public-photo-grid">${p.photos.map(x=>`<figure><img src="${x.dataUrl}"><figcaption>${escapeHtml(x.tag)} ${escapeHtml(x.comment||'')}</figcaption></figure>`).join('')}</div>`}
...
const SCREEN_HELP={
  ...
  clientShare:{title:'元請け向け共有',steps:[['現場を選ぶ','共有する現場を選択。'],['お知らせを入力','元請けに伝えたい内容だけ入力。'],['共有URLを発行','URLをLINE等で送ります。']],note:'最新日報のKY・重点安全目標は共有します。作業者名・社内メモ・原価・引継ぎ先は共有しません。'},
