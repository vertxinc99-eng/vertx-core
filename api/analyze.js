...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const { mode = 'auto', context = '', materialNames = [], learningExamples = [] } = body;
    ...
    const learned = Array.isArray(learningExamples) ? learningExamples.slice(0,8).map((x,i)=>`確定例${i+1}: 条件=${String(x.context||'なし').slice(0,180)} / 確定資材=${JSON.stringify(x.corrected_materials||[]).slice(0,1200)}`).join('\n') : '';
