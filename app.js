...
async function runSystemCheck(){
  ...
  const coreIssues=runCoreSelfCheck();checks.push(['画面・ボタン整合性',coreIssues.length===0,coreIssues.length?coreIssues.slice(0,3).join(' / '):'主要画面・リンクOK']);checks.push(['ビルド',VERTX_BUILD==='7.44.0',`v${VERTX_BUILD}`]);
...
function runCoreSelfCheck(){
  ...
  if(VERTX_BUILD!=='7.44.0')issues.push('ビルド番号不一致:'+VERTX_BUILD);
