...
function canOpenScreen(screenId){const actual=getActualRole();if(screenId==='devTest')return actual==='owner';const r=getEffectiveRole();if(r==='owner')return true;if(r==='admin')return !['plans','devTest'].includes(screenId);if(r==='member')return !['materialsMaster','members','plans','analytics','companySettings','siteCosts','suppliers','auditLog','devTest'].includes(screenId);if(r==='viewer')return ['home','more','history','favorites','drawings','siteStock','siteDashboard','siteQr','companySettings','manual','returnLoad','dailyReport'].includes(screenId);return false}
