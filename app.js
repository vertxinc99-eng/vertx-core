...
async function runAiAnalysis(){
  ...
  const mode=$('#aiMode').value,baseContext=$('#aiContext').value.trim(),field=getLatestDailyForSite(state.selectedSite),fieldContext=field?`\n最新職長日報: 現状=${field.status||''} / KY危険=${field.kyText||''} / KY対策=${field.kyMeasure||''} / 注意=${field.handover||''} / 明日=${field.tomorrow||''} / 必要資材=${field.needed||''}`:'',context=(baseContext+fieldContext).trim(),cacheKey=aiCacheKey(drawings,mode,context),cached=getAiCache(cacheKey);
