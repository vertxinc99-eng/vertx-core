...
function saveOrderMeta(){const v={date:$('#deliveryDate')?.value||'',memo:$('#orderMemo')?.value||'',supplier:$('#confirmSupplier')?.value||''};lsSet(ORDER_META_KEY,JSON.stringify(v))}
function restoreOrderMeta(){const v=getOrderMeta();if($('#deliveryDate'))$('#deliveryDate').value=v.date||$('#deliveryDate').value;if($('#orderMemo'))$('#orderMemo').value=v.memo||'';if($('#confirmSupplier')&&v.supplier)$('#confirmSupplier').value=v.supplier}
function getReturnMeta(){try{return JSON.parse(lsGet(RETURN_META_KEY)||'{}')}catch{return {}}}
function saveReturnMeta(){const v={truck:$('#loadTruckCapacity')?.value||'3000',custom:$('#loadCustomCapacity')?.value||'',site:$('#returnTruckSite')?.value||'',date:$('#returnTruckDate')?.value||'',memo:$('#returnTruckMemo')?.value||''};lsSet(RETURN_META_KEY,JSON.stringify(v))}
...
function canOpenScreen(screenId){const r=getCompanySession()?.role||'member';if(r==='owner')return true;if(r==='admin')return !['plans'].includes(screenId);if(r==='member')return !['materialsMaster','members','plans','analytics','companySettings','siteCosts','suppliers','auditLog'].includes(screenId);if(r==='viewer')return ['home','more','history','favorites','drawings','siteStock','siteDashboard','siteQr','companySettings','manual','returnLoad'].includes(screenId);return false}
...
function startApp(){if(appStarted)return;appStarted=true;const d=new Date();d.setDate(d.getDate()+1);if($('#deliveryDate')&&!getOrderMeta().date)$('#deliveryDate').value=d.toISOString().slice(0,10);restoreOrderMeta();if(state.selectedSite&&$('#siteName'))$('#siteName').value=state.selectedSite;renderCategories();renderMaterials();updateDashboard();toggleAssistOptions();const last=lsGet('vertx_core_last_screen')||'home';go(canOpenScreen(last)?last:'home');prefillSiteFromUrl()}
